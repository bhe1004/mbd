...
The annealed schedule mitigates this sensitivity rather than removing the error.
...
A wide level exposes the optimizer to a broader neighborhood of the current plan, while a narrow level gives the final update a less-smoothed cost and a smaller displacement.
The schedule opens wide and closes narrow, separating exploration from refinement.
This role is most relevant when the rollout is learned, so we compare fixed and annealed schedules under a learned rollout and repeat the schedules on an exact rollout as a control.
...
We run annealed sampling within the control period on a CPU by lifting the measured state once per control step and propagating every candidate in the lifted space, where one rollout step is a fixed number of matrix--vector products.
The full planning computation for \bkmbd{}, including all five annealing stages, candidate generation, lifted rollouts, cost evaluation, softmax weighting, weighted averaging, nominal updates and extraction of $u_0$, returns in $11.0$\,ms median and $14.7$\,ms worst case with no miss of a $50$\,ms period.

\item \textbf{Annealing for learned-rollout robustness.}
We show that fixed-noise sampling must trade broad exploration against low-noise refinement, while a decreasing schedule separates these roles.
Under a learned rollout this separation substantially improves closed-loop accuracy and reduces sensitivity to surrogate-induced minima.
The advantage disappears with exact dynamics, isolating the effect to rollout error rather than to the sampler alone.

\item \textbf{What convexity costs a Koopman predictive controller.}
We isolate the price of convexifying the free space by giving one trained model to two optimizers on a task no single convex region covers.
A quadratic program uses a bilinear lift only after convexification, and it requires convexity of the free space as well.
Holding the model fixed separates that cost from the effects of the model and of the frozen dynamics.
\end{itemize}
...
\begin{lemma}[Irreducible input-channel error of a linear lift~\cite{kim2026bilinear}]
\label{lem:linear}
...
On a plant whose gain varies little the bound is small, and \cite{kim2026bilinear} turns the variation of $\Phi$ into a test on a given operating set.
...
The schedule cannot remove this error from the rollout.
Its role is instead to reduce how strongly one control step depends on a single local view of the surrogate cost.
...
The width a stage explores is therefore set primarily by the noise level.
...
One noise level fixes both quantities, so a fixed-noise sampler must trade exploration against refinement.
A small $\sigma$ gives local proposals, a less-smoothed cost and a small final displacement, but it can remain tied to a surrogate-induced local minimum.
A large $\sigma$ evaluates a broader neighborhood, but it reads a more heavily smoothed cost and produces a coarser update.
This does not establish that a larger noise level is better; it only characterizes the scale at which the stage reads and moves.
The empirical question is whether combining scales is preferable to assigning the same sample budget to one scale.

The annealed schedule separates the two roles.
It opens at a wide level, where the population is spread furthest from the current plan, and closes at a narrow level, where the smoothing and the final displacement are smallest.
Under receding horizon the executed command comes from the last stage.
The early stages therefore expose the optimizer to a broader neighborhood of the current plan, while the last stage refines the executed command under a less-smoothed surrogate cost.
In the single-basin reaching task below, this predicts that the schedule's advantage should be tied to learned-rollout error, a point tested by repeating the schedules with an exact rollout.
...
\dkmbd{} rolls out the linear model \eqref{eq:lin}, whose decoded input sensitivity is the constant matrix that Lemma~\ref{lem:linear} bounds.
...
\dksplit{} also reached $50/50$, and it differs from \dkmbd{} in the input sensitivity alone, so Lemma~\ref{lem:linear} accounts for the failure of \dkmbd{}.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/traj_compare.pdf}
\caption{Executed tool paths on one goal, one of the trials Table~\ref{tab:class} counts. The four runs share the planner, the training seed and the random stream, so they differ in the rollout model alone. \dkmbd{} climbs away from the goal and stalls $0.117$\,m from it, while \dksplit{} ($0.009$\,m), \bkmbd{} ($0.016$\,m) and the oracle \mbdtrue{} ($0.007$\,m) reach the $1$\,cm band.}
\label{fig:paths}
\end{figure}
...
Table~\ref{tab:latency} reports the end-to-end planning time per receding-horizon control step.
The timing includes all five annealing stages, candidate generation, rollout, cost evaluation, softmax weighting, weighted averaging, nominal-sequence updates and extraction of the executed command $u_0$.
...
The \bkmbd{} planner returned at a median of $11.0$\,ms with a worst case of $14.7$\,ms and missed no deadline over all $50$ trials.
...
\centering
\begin{threeparttable}
\caption{End-to-end planning latency per receding-horizon control step over all trials.}
\label{tab:latency}
\footnotesize
...
\end{tabularx}
\begin{tablenotes}[flushleft]\scriptsize
\item Timings include all planner computations through the selected $u_0$; misses count control steps whose planning exceeded the $50$\,ms period.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The reaching task places no obstacle between the start and the goal, so the true cost supplies a control case in which the schedule should not need to overcome a multimodal true landscape.
...
The first question is whether annealing improves on a fixed noise level at the same total sample budget.
Against the single-stage wide and narrow MPPI conditions, the annealed schedule gains $17$ and $35$ goals and loses none.
The second question is whether the gain is merely the result of applying several serial updates.
Against $S$ repeated wide stages and $S$ repeated narrow stages, annealing gains $4$ and $24$ goals.
Multiple stages alone therefore do not account for the result; the changing noise scale is the separating factor.

The third question is whether the gain is tied to learned-rollout error.
Rolling the settled plan of each schedule through the model and through the true dynamics locates where each one stops.
Under the narrow schedule the model puts the terminal error of that plan at $0.024$\,m while the plant delivers $0.053$\,m.
The plan therefore looks locally favorable under the surrogate but substantially worse under the plant.
The annealed schedule returns $0.020$\,m under the model against $0.015$\,m under the plant, so its settled plan lies in a region where the two evaluations agree more closely.

We then repeated two schedules on the true-dynamics rollout, over the same goals and the same planner stream.
The schedules stop separating in success rate.
The narrow schedule reaches the $1$\,cm band on $10/10$ goals as the annealed schedule does, against $26/50$ under the learned rollout, and its settled error moves from $0.045$ to $0.009$\,m.
The narrow level settles quickly on either rollout, so what it loses under the learned one is not an inability of the sampler to refine a plan.
It is sensitivity to the surrogate landscape.
...
This trend is consistent with the interpretation of Sec.~\ref{subsec:anneal}: larger noise produces broader and coarser updates, and stronger smoothing makes them less suited to final refinement.
The wide schedules therefore come to rest at the edge of the band without settling inside it.
...
Its displacement coincides with the narrow level because the final stage produces the executed step, while the preceding wider stages have already exposed the iterate to a broader neighborhood of the current plan.
