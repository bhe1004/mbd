"""Run the 7-DOF arm BK-MBD comparison.
...
`experiments/train_arm_koopman.py`. For `bk_mbd`, tube constants are fitted
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=PROJECT_ROOT / "out" / "arm"
    )
...
def main() -> None:
    ...
    if args.method in koopman_methods:
        ...
        checkpoint = args.checkpoint or (
            PROJECT_ROOT / "out" / "arm" / "models" / f"{short}_seed{args.seed}.pt"
        )
        if not checkpoint.exists():
            raise SystemExit(
                f"checkpoint not found: {checkpoint}\n"
                "train it first: python experiments/train_arm_koopman.py"
            )
