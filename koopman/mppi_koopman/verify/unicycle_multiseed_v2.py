"""Improved unicycle multi-seed: better MPPI tuning (Ks 1536, more steps) and several start/goal
pairs, with the ORACLE run on every trial to confirm the task itself is reliably solvable.
Reports park rate for ours/MPPI-DK/oracle over (seeds x pairs). v1's deep MPPI was marginal (oracle failed once)."""
...
def mppi(mode, model, s0, goal, T=40, Ks=1536, lam=0.4, sig=(0.6, 0.9), w_du=0.4, steps=180, seed=3):
    ...
    u_prev = torch.zeros(2); gT = torch.tensor(goal); sig = torch.tensor(sig)
    for t in range(steps):
        ...
        if np.linalg.norm(s[:2].numpy()-goal[:2]) < 0.08 and abs(((s[2].item()-goal[2]+np.pi) % (2*np.pi))-np.pi) < 0.2:
            break
    return float(np.linalg.norm(s[:2].numpy()-np.array(goal[:2])))
PAIRS = [[1.5, 1.5, -np.pi/2], [-1.5, 1.0, 0.0], [1.0, -1.5, np.pi/2], [-1.2, -1.2, np.pi]]
goal = [0.0, 0.0, 0.0]; SEEDS = [0, 1, 2, 3, 4]; PARK = 0.3
nP = len(PAIRS); nS = len(SEEDS)
ours = np.zeros((nS, nP)); lin = np.zeros((nS, nP)); orc = np.zeros((nS, nP))
for si, seed in enumerate(SEEDS):
    torch.manual_seed(seed); np.random.seed(seed)
    m_bi = train(True); m_lin = train(False)
    for pj, s0 in enumerate(PAIRS):
        ours[si, pj] = mppi("bilinear", m_bi, s0, goal, seed=seed*10+pj)
        lin[si, pj] = mppi("linear", m_lin, s0, goal, seed=seed*10+pj)
        orc[si, pj] = mppi("true", None, s0, goal, seed=seed*10+pj)
    print(f"seed {seed}: ours park {int((ours[si]<PARK).sum())}/{nP}  MPPI-DK {int((lin[si]<PARK).sum())}/{nP}  oracle {int((orc[si]<PARK).sum())}/{nP}")
tot = nS*nP
print(f"\n[v2 improved] park rate (<{PARK} m) over {nS} seeds x {nP} start/goal = {tot} trials:")
print(f"  ours    : {int((ours<PARK).sum())}/{tot}  ({100*(ours<PARK).mean():.0f}%)  final {ours[ours<PARK].mean():.3f} m (parked)")
print(f"  MPPI-DK : {int((lin<PARK).sum())}/{tot}  ({100*(lin<PARK).mean():.0f}%)  final {lin.mean():.2f}+/-{lin.std():.2f} m")
print(f"  oracle  : {int((orc<PARK).sum())}/{tot}  ({100*(orc<PARK).mean():.0f}%)  final {orc[orc<PARK].mean() if (orc<PARK).any() else float('nan'):.3f} m (parked)")
