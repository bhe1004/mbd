"""Regenerate the unicycle parking figure from the DEEP Koopman model with the v2 MPPI tuning,
so the figure matches the multi-seed statistics (deep model, 95% vs 0%). One representative
trial (seed 0): ours parks, oracle parks, MPPI-DK drives off. Saves template/figs/fig_unicycle.png."""
...
K = 15
print("data + train (seed 0)..."); Btr, Utr = make_snips(8000, K, 1)
...
@torch.no_grad()
def mppi(mode, model, s0, goal, T=40, Ks=1536, lam=0.4, sig=(0.6, 0.9), w_du=0.4, steps=180, seed=3):
    g = torch.Generator().manual_seed(seed); s = torch.tensor(s0); U = torch.zeros(T, 2)
    u_prev = torch.zeros(2); gT = torch.tensor(goal); sig = torch.tensor(sig); traj = [s.numpy().copy()]; stop = steps
...
runs = [("Ours: bilinear Koopman MPPI", lambda: mppi("bilinear", m_bi, s0, goal, seed=0), "#1f77b4"),
        ("MPPI-true: oracle (true dynamics)", lambda: mppi("true", None, s0, goal, seed=0), "#2ca02c"),
        ("MPPI-DK: linear Koopman MPPI", lambda: mppi("linear", m_lin, s0, goal, seed=0), "#d62728")]
PARK = 0.3; lim = 2.6
fig, axes = plt.subplots(1, 3, figsize=(15, 4.6))
for ax, (title, fn, col) in zip(axes, runs):
    tr, stop = fn(); d = float(np.linalg.norm(tr[-1, :2]-np.array(goal[:2])))
    ...
    ax.arrow(goal[0], goal[1], 0.32, 0.0, head_width=0.13, head_length=0.13, fc="k", ec="k", zorder=4)
    ax.scatter([s0[0]], [s0[1]], c="0.5", marker="o", s=70, zorder=4, label="start")
    ax.plot(tr[:, 0], tr[:, 1], "-", c=col, lw=2.2, alpha=0.9)
    sf = tr[-1]; ax.arrow(sf[0], sf[1], 0.38*np.cos(sf[2]), 0.38*np.sin(sf[2]), head_width=0.17, head_length=0.17, fc=col, ec=col, lw=2, length_includes_head=True, zorder=5)
    tag = "  PARKED" if d < PARK else ""
    ax.text(-lim+0.12, lim-0.28, f"step {stop:3d}   dist {d:.2f}{tag}", fontsize=9.5, family="monospace")
    ax.legend(loc="lower right", fontsize=9)
    print(f"{title.split(':')[0]:9s}: dist {d:.3f}  {'PARKED' if d<PARK else 'fail'}")
plt.tight_layout(); plt.savefig(os.path.join(FIGS, "fig_unicycle.png"), dpi=150)
print("saved /figs/fig_unicycle.png (deep model, v2 tuning)")
