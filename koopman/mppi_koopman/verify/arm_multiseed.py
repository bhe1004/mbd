"""Multi-seed robustness of the 7-DOF arm reach result: retrain ours/MPPI-DK over several seeds,
run all 7 targets each, report reach counts mean+-std and per-target error bars. Preempts
'6/7 is a cherry-picked seed'. Data generated once; only the training seed (init + shuffle) varies."""
...
print("data (once)..."); Btr, Utr = gen(4500, 1)
class DK(nn.Module):
...
for si, seed in enumerate(SEEDS):
    ...
    nb = int((bi_err[si] < THR).sum()); nl = int((lin_err[si] < THR).sum())
    print(f"seed {seed}: ours {nb}/{nt}  MPPI-DK {nl}/{nt}")
bi_reach = (bi_err < THR).sum(1); lin_reach = (lin_err < THR).sum(1)
print(f"\nReach (<{THR}) over {len(SEEDS)} seeds:")
...
print(f"  oracle  : 7/7 (det.)")
print(f"  ours final err  mean {bi_err.mean():.3f} (reached-only {bi_err[bi_err<THR].mean():.3f})")
print(f"  MPPI-DK final err mean {lin_err.mean():.3f} +/- {lin_err.std():.3f}")
...
print("saved fig_arm_bar.png (mean+-std over seeds)")
